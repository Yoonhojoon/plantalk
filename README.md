...
| 엔드포인트 | 메서드 | 설명 | 요청 본문 | 응답 |
|------------|--------|------|------------|------|
| /api/species | GET | 식물 종 목록 조회 | - | [{ id, name, scientific_name, description, image_url, temp_range_min, ... }]

### 4. 식물 (Plants)

| 엔드포인트 | 메서드 | 설명 | 요청 본문 | 응답 |
|------------|--------|------|------------|------|
| /api/plants | GET | 사용자의 식물 목록 조회 | - | [{ id, user_id, species_id, name, location, ... }] |
| /api/plants | POST | 새 식물 등록 | { species_id?, name, location, watering_cycle_days, last_watered_at?, temp/humidity/light ranges? } | 식물 객체 |
| /api/plants/{plant_id} | GET | 특정 식물 정보 조회 | - | 식물 객체 |
| /api/plants/{plant_id} | PATCH | 식물 정보 수정 | { name?, location?, watering_cycle_days?, 기준값들 } | 식물 객체 |
...
| 엔드포인트 | 메서드 | 설명 | 요청 본문 | 응답 |
|------------|--------|------|------------|------|
| /api/notifications | GET | 사용자의 알림 목록 조회 | - | [{ id, user_id, plant_id, title, type, status, created_at, read_at }] |
...
- 모든 API 요청에는 인증이 필요합니다.  
  Authorization 헤더에 `Bearer {access_token}` 형식으로 토큰을 포함해야 합니다.
- `location` 필드는 다음 값들만 허용됩니다: `"거실"`, `"침실"`, `"주방"`, `"화장실"`, `"베란다"`, `"정원"`, `"실내"`, `"실외"`
- `species_id`를 제공하지 않는 경우, 모든 기준값(`temp_range_min/max`, `humidity_range_min/max`, `light_range_min/max`)을 직접 입력해야 합니다.
